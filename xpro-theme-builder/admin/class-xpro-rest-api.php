...
final class Xpro_Theme_Builder_Rest_Api {
	...
	public static function create_post( $request ) {
		...
		update_post_meta( $post, '_elementor_template_type', 'xpro-themer' );
	...
	public static function get_single_post( $request ) {
		...
		if ( $query->have_posts() ) {
			...
			$posts_data[] = (object) array(
				...
				'all_rules'      => Xpro_Target_Rules_Fields::get_user_selections(),
	...
	public static function update_post( $request ) {
		...
		update_post_meta( $id, '_elementor_template_type', 'xpro-themer' );
